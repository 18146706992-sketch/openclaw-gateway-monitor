...
# Priority:
#   1. User-provided -LogDir parameter
#   2. OPENCLAW_STATE_DIR environment variable + "\logs\openclaw-monitor"
#   3. Script's parent directory + "\logs"
# ============================================================
if ($LogDir -eq "") {
    if ($env:OPENCLAW_STATE_DIR) {
        $LogDir = Join-Path $env:OPENCLAW_STATE_DIR "logs\openclaw-monitor"
    } else {
